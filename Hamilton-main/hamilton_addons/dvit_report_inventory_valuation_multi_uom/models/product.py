...
class ProductTemplate(models.Model):
    ...
    available_uom_ids = fields.Many2many('uom.uom', string='Available Uom')
    available_uom_domain_ids = fields.Many2many('uom.uom', store=False,
                                                readonly=True,
    ...
    def _compute_available_uom_domain(self):
        for record in self:
            if record.uom_id:
                filter_by = self.env['uom.uom'].sudo().search(

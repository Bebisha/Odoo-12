...
class AccountInvoice(models.Model):
    ...
    def l10n_co_edi_upload_electronic_invoice(self):
        ...
        if to_process:
            ...
            if to_process.filtered(lambda m: m.date_invoice and not (oldest_date <= fields.Datetime.to_datetime(m.date_invoice) <= newest_date)):
    ...
    def _l10n_co_edi_generate_xml(self):
        ...
        retention_taxes_dict = defaultdict(list)
        for tax in retention_taxes:
            retention_taxes_dict[tax.tax_id.l10n_co_edi_type].append(tax)
        regular_taxes = tax_lines_with_type - retention_taxes
        regular_taxes_dict = defaultdict(list)
        for tax in regular_taxes:
            regular_taxes_dict[tax.tax_id.l10n_co_edi_type].append(tax)

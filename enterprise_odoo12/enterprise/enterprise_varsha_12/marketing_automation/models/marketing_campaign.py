...
class MarketingCampaign(models.Model):
    ...
    @api.onchange('marketing_activity_ids')
    def _check_mass_mailing_compatible(self):
        for campaign in self:
            for activity in campaign.marketing_activity_ids:
                if activity.mass_mailing_id:
                    fields = self.env[activity.mass_mailing_id.mailing_model_real]._fields
                    field = fields.get('email', fields.get('email_from', fields.get('partner_id')))
                    if not field or not field.store:
                        raise ValidationError(_("Unsupported mass mailing model %s") % activity.mass_mailing_id.mailing_model_id.name)

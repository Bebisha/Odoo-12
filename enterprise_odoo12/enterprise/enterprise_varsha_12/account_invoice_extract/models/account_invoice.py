...
class AccountInvoice(models.Model):
    ...
    @api.multi
    def _set_bank_account(self, iban_ocr):
        self.ensure_one()
        if self.partner_id:
            bank_account = self.env['res.partner.bank'].search([('partner_id', '=', self.partner_id.id), ('acc_number', '=ilike', iban_ocr)])
    ...
    def _fill_invoice_with_ocr_values(self, record, ocr_results):
        ...
        if iban_ocr and not record.partner_bank_id:
            record._set_bank_account(iban_ocr)

...
class ProductPack(models.Model):
    ...
    price = fields.Float(related='product_id.standard_price', string='Product Price')
...
class ProductProduct(models.Model):
    ...
    stock_location_ids = fields.Many2many('stock.location', 'product_stock_loc_rel', 'prod_id', 'stock_id',
                                          string='Stock Location', compute=_default_location)

    @api.depends('pack_ids','product_variant_ids', 'product_variant_ids.standard_price')
    def _compute_standard_price(self):
        price = []
        for product in self:
            if product.pack_ids:
                for pack in product.pack_ids:
                    price.append(pack.qty_uom * pack.price)
                standard_price = sum(price)
        ...
        if (rslt.payment_method_id.name == 'PDC'):
            ...
            for user in acc_user:
                user_id = self.env['mail.followers'].search(
                    [('res_id', '=', user.id), ('res_model', '=', 'account.payment'),
